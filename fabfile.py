...
def rollback():
    ...
    with cd(_REMOTE_BASE_DIR):
        ...
        print('==================================================')
        print('')
        yn = input('continue? y/N ')  # 将raw_input改为input， 这个应该是python2 和Python3的区别。
        if yn != 'y' and yn != 'Y':
...
def restore2local():
    ...
    print('')
    try:
        num = int(input('Restore file: '))  # 将raw_input改为input， 这个应该是python2 和Python3的区别。
    except ValueError:
        print('Invalid file number.')
        return
    restore_file = files[num]
    yn = input('Restore file %s: %s? y/N ' % (num, restore_file))  # 将raw_input改为input， 这个应该是python2 和Python3的区别。
    if yn != 'y' and yn != 'Y':
        print('Restore cancelled.')
        return
    print('Start restore to local database...')
    p = input('Input mysql root password: ')  # 将raw_input改为input， 这个应该是python2 和Python3的区别。
    sqls = [
